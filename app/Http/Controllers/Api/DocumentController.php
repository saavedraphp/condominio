...
use Illuminate\Support\Facades\Auth; // Para verificar autenticación si es necesario
use Symfony\Component\HttpFoundation\StreamedResponse; // Para descarga

class DocumentController extends Controller
{
    /**
     * Lista los documentos visibles para los residentes.
     */
    public function index(Request $request)
    {
        ...
        // Asumiendo que cualquier usuario autenticado puede ver la lista de residentes
        $documents = Document::where('is_visible', true)
            ->select('id', 'title', 'type', 'created_at') // Campos para la lista
            ->orderBy('created_at', 'desc')
            ->paginate(15); // Opcional: paginación
